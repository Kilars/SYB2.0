        ...
        public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var match = await context.TournamentMatches
                ...
                .FirstOrDefaultAsync(m =>
                    m.TournamentId == request.TournamentId && m.MatchNumber == request.MatchNumber, cancellationToken);
            ...
            {
                int nextRound = match.BracketRound + 1;
                int nextPosition = (match.BracketPosition + 1) / 2;
                var nextMatch = await context.TournamentMatches
                    .FirstOrDefaultAsync(m =>
                        m.TournamentId == tournament.Id
                        && m.BracketRound == nextRound
                        && m.BracketPosition == nextPosition, cancellationToken);
