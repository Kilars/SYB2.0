...
_Backlog empty. Run `backlog-scan` or `feature-planning` to queue new work._

## Recently Completed

1. **048-TEST-3p-league-lifecycle-e2e** — 3-player league lifecycle E2E test. Scope expanded (user-approved): the original task was test-only but no UI path existed to activate an N>2 league. Added the missing league player-count activation UI to `StatusButton.tsx` (PlayerCountToggle rendered while Planned; `{status:1, playerCount}` sent on Planned→Active only; backend/`useLeagues.updateStatus` already supported it). New `e2e/tests/lifecycle/league-3p-lifecycle.spec.ts` (6 serial tests: create → activate N=3 → 4 matches → PodiumPicker completion → placement-points leaderboard → cumulative rotation → stats smoke), new `podium-picker.page.ts` page object, `expectedMatchCount(v,n)` helper, and `leaderboard.page.ts` `getPerformanceColumnHeader`/`getPlayerRowByUserId`. Deviations from AC (verified): FfaMatchForm is podium-only (no character picker), submit button is "Complete FFA Match", player-count chosen at activation. Client build + lint pass; e2e run deferred (no harness — precedent 044/047). /simplify pass applied (dedup page-object instantiation). Completed 2026-05-15.
2. **046b-FEATURE-tournament-n-player-frontend** — Tournament N-player frontend + runtime advancement. New `SlotMapping` pure function (0-based positionInRound; N=2/3 top-1, N=4 top-2 cross-pair) plus shared `SlotMapping.SetSlot` used by both `CompleteTournamentMatch` (rewritten: N-branched, async `AdvanceAdvancers`, placements written to Match row) and `ReopenTournamentMatch` (rewritten: symmetric SlotMapping clear, placement-column reset, transitive-safety). `RoundDto` extended with Second/Third/FourthPlaceUserId; `CompleteTournamentMatchValidator` rewritten with N=2 vs N>2 branching. Frontend: `Tournament.perHeatPlayerCount` type, `tournamentSchema` BestOf coupling, `useFfaMatch` `mode:'tournament'`, `FfaMatchForm` mode prop, `CompetitionForm` PlayerCountToggle + legal-roster helper, `BracketView` mobile scroll-snap + N-participant heat cards with `PodiumDisplay`, `TournamentList` format chip, new shared `bracketSizing.ts` util. Auth handlers audited (participant-agnostic, no change). dotnet build/tests deferred (no harness — precedent 043/044/045/046a); frontend build + lint pass. /simplify pass applied (N+1 fix, dead-code removal, slot-helper consolidation, async advancement, table dedup). Completed 2026-05-15.
2. **046a-FEATURE-tournament-n-player-backend** — Tournament N-player backend groundwork. Added `Tournament.PerHeatPlayerCount` (int NOT NULL DEFAULT 2, hand-written migration `AddTournamentPerHeatPlayerCount` + AppDbContext registration + snapshot). New shared `BracketSizing` (integer-safe switch tables — valid sizes/total rounds/advance ratio/advancers per heat, NO Math.Log) and `BracketBuilder` (single-elim generator: round-robin seed distribution across heats, randomized in-heat positional slot fill, globally monotonic MatchNumber for 046b's SlotMapping). `StartTournament` and `ShuffleBracket` refactored to delegate to `BracketBuilder` (inline duplicated bracket loops removed); `StartTournament` enforces exact-size roster (no CPU padding) with a legal-counts error message. `CreateTournament`/validator/DTOs carry `PerHeatPlayerCount` (2..4; BestOf forced to 1 when N>2; BracketSize initialized to N at create to avoid log2(0) pathologies). `/simplify` pass applied comment cleanups; build/tests deferred (no dotnet/test harness — precedent 043/044/045). Frontend + advancement logic deferred to 046b. Completed 2026-05-15.
