...
export default function UserSelectInput<T extends FieldValues>(props: Props<T>) {
  ...
  const availableUsersList: User[] = props.users?.filter(
    (user) => !members?.map((members) => members.userId).includes(user.id),
  );
  ...
  const addMember = (event: SelectChangeEvent) => {
    ...
    if (addUser)
      field.onChange([...members, { userId: addUser.id, displayName: addUser.displayName }]);
  };
          ...
          {availableUsersList.map((user) => (
            <MenuItem key={user.id} value={user.id}>
              <Box display="flex" alignItems="center" sx={{ height: "100%" }}>
                ...
                <Typography variant="body1">{getDisplayLabel(user)}</Typography>
              </Box>
            </MenuItem>
          ))}
        </Select>
