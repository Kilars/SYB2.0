...
function HeroSection() {
  ...
  return (
    ...
    >
      ...
      <Typography variant="h3" sx={{ fontSize: { xs: '1.75rem', sm: '3rem' }, textAlign: 'center' }}>
        Smash Your Bros
      </Typography>
      <Typography variant="h5" textAlign="center" sx={{ opacity: 0.9, fontSize: { xs: '1rem', sm: '1.5rem' } }}>
...
function QuickStats({ userId }: { userId: string }) {
  ...
  const stats = [
    ...
    { label: 'Pending', value: pendingMatches, color: SMASH_COLORS.p3Yellow, icon: <SportsEsports /> },
  ];

  return (
    <Box sx={{ mb: 4 }}>
      <Typography variant="h6" fontWeight="bold" mb={2}>Your Stats</Typography>
        ...
        {stats.map(stat => (
          <Card key={stat.label} sx={{
            ...
            <CardContent sx={{ textAlign: 'center', py: { xs: 1.5, sm: 2 } }}>
              ...
              <Typography variant="h4" fontWeight="bold" sx={{ color: stat.color, fontSize: { xs: '1.5rem', sm: '2rem' } }}>
                {stat.value}
              </Typography>
                ...
                <Box display="flex" justifyContent="space-between" alignItems="center">
                  ...
                  {match.completed ? (
                    <Chip
                      ...
                      sx={{
                        fontWeight: 'bold',
                        backgroundColor: won ? `${SMASH_COLORS.p4Green}22` : `${SMASH_COLORS.p1Red}22`,
...
function ActiveLeagues() {
  ...
  if (isLeaguesLoading) {
    return (
      <Box sx={{ mb: 4 }}>
        <Typography variant="h6" fontWeight="bold" mb={2}>Active Leagues</Typography>
        ...
        {activeLeagues.map(league => {
          ...
          return (
            ...
            >
              <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 } }}>
                <Box display="flex" justifyContent="space-between" alignItems="center">
                  <Typography fontWeight="bold" noWrap sx={{ flex: 1, minWidth: 0 }}>{league.title}</Typography>
                  <Chip
